...
const HomePageReason = (props) => {
  ...
  const tempComponent = (props) => {
    return <div data-aos = {props.animation} data-aos-easing="ease-in-out" data-aos-once="true">
    <div className = {`tw-flex tw-items-center tw-mx-auto tw-max-w-4xl tw-py-16 ${props.reverse === false ? '': 'tw-flex-row-reverse'}`} >
      <img src= {props.img} width= '300px' height='300px'></img>
      <div className = 'tw-px-16' >
        <div className='tw-font-bold tw-text-4xl tw-text-black'>{props.title}</div>
        <div className= 'tw-font-medium tw-text-3xl tw-text-black tw-text-truncate tw-mt-7'> {props.reason}</div>    
      </div>
    </div>
    <div className= {`HomePageDraw ${props.draw === false ? '' : 'tw-flex tw-mx-auto tw-max-w-5xl tw-h-1 tw-bg-gray-200 tw-border-0'}`}></div>
  </div>
  }
  const DataReasons = [
    {
